...
func (*ITrain) GetAllTrains() ([]data.Train, error) {
	rows, err := GetDBManager().Db.Query("select * from public.trains")
	if err != nil {
		return nil, err
	}
...
func (*ITrain) GetTrain(id string) (data.Train, error) {
	...
	row := GetDBManager().Db.QueryRow("select * from trains where id = $1", idint)
	t := data.Train{}
...
func (*ITrain) AddTrain(t data.Train) error {
	_, err := GetDBManager().Db.Exec("insert into trains (departure_city,arrival_city,departure_time,departure_date,arrival_time,arrival_date) values ($1,$2,$3,$4,$5,$6)", t.DepartureCity, t.ArrivalCity, t.DepartureTime, t.DepartureDate, t.ArrivalTime, t.ArrivalDate)
...
func (*ITrain) UpdateTrain(t data.Train) error {
	_, err := GetDBManager().Db.Exec("update public.trains set departure_city = $1 , arrival_city = $2, departure_time = $3, departure_date = $4, arrival_time = $5, arrival_date = $6 where id = $7", t.DepartureCity, t.ArrivalCity, t.DepartureTime, t.DepartureDate, t.ArrivalTime, t.ArrivalDate, t.ID)
...
func (*ITrain) DeleteTrain(id uuid.UUID) error {
	_, err := GetDBManager().Db.Exec("delete from trains where id = $1", id)
	if err != nil {
		return err
	}
	return nil
